...
function validatePredictionData(data: unknown): {
  valid: boolean;
  error?: string;
  parsed?: number[];
} {
...
async function callVercelPythonFunction(
  type: string,
  data: number[]
): Promise<unknown> {
  try {
    ...
    if (type === "fee") {
      ...
      const priorityStr =
        priority > 0.7 ? "high" : priority < 0.3 ? "low" : "normal";

      const result = await realMLServices.predictRealFee(
        amount,
        "APT",
        priorityStr
      );
